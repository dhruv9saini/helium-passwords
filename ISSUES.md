...
| HP-010 | P1 | Implemented; live delivery acceptance pending | Chromiumer terminal state is observed by a persistent lm systemd timer and idempotently queued through Codex Mailbridge for success, failure, timeout, or cancellation. | Source simulations cover all terminal results plus a temporary queue failure, one constrained live job reaches Mailbridge Sent exactly once, and service recovery remains enabled across lm restart. |
