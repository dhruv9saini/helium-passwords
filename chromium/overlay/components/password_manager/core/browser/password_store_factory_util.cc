...
#if BUILDFLAG(IS_ANDROID)
constexpr base::FilePath::CharType kAndroidLoginDataForProfileFileName[] =
    FILE_PATH_LITERAL("Login Data");
constexpr base::FilePath::CharType kAndroidLoginDataForAccountFileName[] =
    FILE_PATH_LITERAL("Login Data For Account");

void AndroidFactoryLog(const std::string& message) {
  __android_log_write(ANDROID_LOG_WARN, "HeliumSync", message.c_str());
}
#else
void AndroidFactoryLog(const std::string&) {}
#endif
...
std::unique_ptr<LoginDatabase> CreateLoginDatabase(
  ...
  base::FilePath login_db_file_path =
      db_directory.Append(is_account_store
#if BUILDFLAG(IS_ANDROID)
                              ? kAndroidLoginDataForAccountFileName
                              : kAndroidLoginDataForProfileFileName
#else
                              ? kLoginDataForAccountFileName
                              : kLoginDataForProfileFileName
#endif
      );
  AndroidFactoryLog("password db " +
                    std::string(is_account_store ? "account " : "profile ") +
                    login_db_file_path.AsUTF8Unsafe());
