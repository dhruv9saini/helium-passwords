...
| Enrollment | d-only seed, hash-only registry bootstrap, pending pull-only join phase, native password cursor gate, revocation and bearer rotations; cookies and tabs cannot affect promotion | Execute the password-only receipt gate on disposable Linux and Android clients, then enroll the personal OnePlus only after the disposable package and all recovery gates pass |
...
| Cookies | No Tailnet transport or normal-profile reconciler; fixed marker-gated CookieManager neutral snapshot/import/readback/rollback; stopped compressed full-profile archives; independent encrypted restic repositories owned by da | Compile the fixture, then pass C1 neutral, C2 compressed profile, and C3 encrypted repository restores with synthetic disposable cookies only; never inspect or restore personal Google cookies |
| Origin state | Strict metadata-only, artifact-bound synthetic/disposable classifier; no source-registered transfer adapter and no state values accepted | Keep localStorage, IndexedDB, service workers, Cache Storage, and arbitrary site databases outside this product |
...
| Media/streaming | Reproducible fixtures, strict codec GN provenance, a separate no-patch upstream control, and emulator/device-test tooling retained as supplemental browser QA | Do not use OnePlus or lm for assistant browser control; password completion does not depend on the older OnePlus CDP media gate |
| Android source/build | Exact Chromium `150.0.7871.181`/Helium `0.14.8` lock; shared one-request immutable source helper; exact-HEAD, depot pin, cache-disable, monotonic version, and public Passwords overlay contract | Fresh isolated chromiumer source preparation, composition, GN generation, focused compile, APK verification, disposable OnePlus installation, then personal-package installation and enrollment |
