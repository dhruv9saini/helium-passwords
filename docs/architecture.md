...
| Enrollment | d-only seed, hash-only registry bootstrap, pending pull-only join phase, dual bridge cursor gate, revocation and bearer rotations | Execute native bridge promotion on disposable profiles, then provision personal d/da/oneplus only after backups and explicit approval |
