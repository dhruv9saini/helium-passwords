...
| Enrollment | d-only seed, hash-only registry bootstrap, pending pull-only join phase, dual bridge cursor gate, revocation and bearer rotations; the artifact-bound three-client receipt gate is source-ready | Execute the receipt gate with native bridge promotion on three disposable clients, then provision personal d/da/oneplus only after backups and explicit approval |
