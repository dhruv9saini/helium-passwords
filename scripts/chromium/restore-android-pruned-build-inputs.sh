...
if [[ $# -ne 4 ]]; then
  echo "usage: $0 <restore|validate> CHROMIUM_SRC EXPECTED_COMMIT PRUNING_LIST" >&2
  exit 64
fi
...
case "$action" in
  restore|validate) ;;
  *)
    echo "unknown Android build-input action: $action" >&2
    exit 64
    ;;
esac
...
for relative_path in "${readonly_paths[@]}"; do
  ...
  if [[ "$action" == restore ]]; then
    ...
    [[ -f "$source_root/$relative_path" && ! -L "$source_root/$relative_path" ]] || {
      echo "restored Android build input is not a regular file: $relative_path" >&2
      exit 1
    }
  fi
  ...
  printf '%s_android_build_input=%s blob=%s\n' \
    "$action" "$relative_path" "$expected_blob"
done
