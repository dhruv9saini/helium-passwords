...
test_root=$(mktemp -d /tmp/helium-android-pruned-prefs.XXXXXX)
cleanup() {
...
for relative_path in \
  ...
  components/safe_browsing/core/common/safe_browsing_prefs.h \
  components/signin/public/base/signin_pref_names.cc \
  components/signin/public/base/signin_pref_names.h; do
...
[[ "$(grep -c '^restored_android_pref_input=' <<< "$restore_output")" -eq 4 ]]
find "$source_root" -type f -name '*BUILD.gn' -print0 | sort -z | \
...
if "$repo_root/scripts/chromium/restore-android-pruned-pref-inputs.sh" \
  "$source_root" "$pinned_commit" "$pruning_list" \
  > "$test_root/repeated.out" 2>&1; then
  echo 'repeated Android pref restoration unexpectedly passed' >&2
  exit 1
fi
...
echo 'Android pruned pref restoration and generator inventory passed'
