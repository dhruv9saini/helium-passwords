...
if [[ $# -ne 3 ]]; then
  echo "usage: $0 ANDROID_ARCHIVE EXPECTED_HELIUM_SYNC_COMMIT NEW_OUTPUT_DIRECTORY" >&2
  exit 64
fi

repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)
archive=$(realpath "$1")
expected_sync_commit=$2
output=$(realpath -m "$3")
...
"$repo_root/scripts/chromium/verify-android-artifact.sh" \
  "$archive" computer.helium.sync.test "$expected_sync_commit" > "$verify_result"

extracted="$temporary/extracted"
staged="$temporary/staged"
mkdir -p "$extracted" "$staged"
tar -xf "$archive" -C "$extracted"
mapfile -d '' apks < <(find "$extracted" -type f -name HeliumSync.apk -print0)
[[ ${#apks[@]} -eq 1 ]] || { echo "verified archive lost its unique APK" >&2; exit 1; }

install -m 600 "${apks[0]}" "$staged/HeliumSync-test.apk"
...

{
  ...
  printf 'package=computer.helium.sync.test\n'
  ...
  printf 'apk_sha256=%s\n' "$(sha256sum "$staged/HeliumSync-test.apk" | cut -d' ' -f1)"
