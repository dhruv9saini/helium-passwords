...
if [[ -n "$adb_serial" ]]; then
  [[ "$adb_serial" == oneplus:5555 ]] || {
    echo "Android enrollment requires the fixed oneplus ADB endpoint" >&2
    exit 64
  }
...
uid=$("${adb_device[@]}" shell "cmd package list packages -U '$package'" | tr -d '\r' | sed -n "s/^package:$package uid://p" | head -n1)
...
if [[ "$package" == computer.helium.sync.test ]]; then
  ...
  current_tree_sha=$("${adb_device[@]}" exec-out run-as "$package" sh -c \
    "cd '$data_dir' && exec /system/bin/tar -cf - app_chrome" |
    sha256sum | awk '{print $1}')
else
  current_tree_sha=$("${adb_device[@]}" exec-out /debug_ramdisk/su -c \
    "cd '$data_dir' && /system/bin/tar -cf - app_chrome" |
    sha256sum | awk '{print $1}')
fi
...
cleanup() {
  rm -rf -- "$work_dir"
  "${adb_device[@]}" shell "rm -f '/data/local/tmp/$bundle_name'" >/dev/null 2>&1 || true
}
trap cleanup EXIT
...
" <"$work_dir/enrollment.tar" >/dev/null
else
  "${adb_device[@]}" push "$work_dir/enrollment.tar" "/data/local/tmp/$bundle_name" >/dev/null
  "${adb_device[@]}" shell '/debug_ramdisk/su -c "
set -eu
DATA='"'"$data_dir"'"'
UID_NUMBER='"'"$uid"'"'
